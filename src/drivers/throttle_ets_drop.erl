...
reset_counters(Scope) ->
  [{Scope, OldTableId, Limit, Period, _PreviousReset}] = ets:lookup(?STATE_TABLE, Scope),
  NewTableId = ets:new(scope_counters, [set, public]),
  true = ets:insert(?STATE_TABLE, {Scope, NewTableId, Limit, Period, timestamp()}),
  ets:delete(OldTableId),
  ok.
